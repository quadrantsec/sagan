...
void Alert_Syslog( _Sagan_Event *Event )
{
    ...
    char *syslog_template = "[1:%" PRIu64 ":%d] %s [Classification: %s] [Priority: %d] [Program: %s] %s %s:%d -> %s:%d - %s";
    ...
    snprintf(syslog_message_output, sizeof(syslog_message_output), syslog_template, Event->sid, Event->rev, Event->f_msg, classbuf, Event->pri, Event->program, tmp_proto, Event->ip_src, Event->src_port, Event->ip_dst, Event->dst_port, Event->message);
